...
DROP TABLE IF EXISTS `Combined_tripdata_2022.cleaned_combined_data`;


-- Creating a new table with clean data

CREATE TABLE IF NOT EXISTS `Combined_tripdata_2022.cleaned_combined_data` AS(
  SELECT 
  ...
  FROM `Combined_tripdata_2022.combined_data` a
  JOIN (
    SELECT ride_id, (
...
ALTER TABLE `Combined_tripdata_2022.cleaned_combined_data` -- set ride_id as primary key
